...
export default function Dashboard() {
  ...
  // 1. Fixed Class List (PG, LKG, UKG to Class 12)
  const schoolClasses = [
    "PG", "LKG", "UKG", 
  ...
  // Date Formatting for Display and Firebase Keys
  ...
  // 3. Real-time Data Sync (Filtering Deleted Students)
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "students"), (snap) => {
      const list = snap.docs
        .map(d => ({ id: d.id, ...d.data() }))
        .filter(s => !s.deletedAt); // 🔥 Sirf wahi jo delete nahi huye
      setStudents(list);
    });
    return () => unsub();
  }, []);

  // 4. Multi-Filter Logic (Session + Class)
  ...
  const presentCount = filteredStudents.filter((s) => {
    ...
    return s.attendance?.[currentMonth]?.[dayKey] === "P";
  }).length;

  const absentCount = filteredStudents.filter((s) => {
    const dayKey = `${currentMonth}_day_${currentDay}`;
    return s.attendance?.[currentMonth]?.[dayKey] === "A";
  }).length;

  // 6. Fees Calculation (paidSchool + paidBus)
  const totalFeesExpected = filteredStudents.reduce((sum, s) => {
    months.forEach(m => { sum += (s.fees?.[m]?.total || 0); });
    return sum;
  }, 0);

  const totalCollection = filteredStudents.reduce((sum, s) => {
    months.forEach(m => { 
      const paid = (s.fees?.[m]?.paidSchool || 0) + (s.fees?.[m]?.paidBus || 0);
      sum += paid; 
    });
    return sum;
  }, 0);

  const pendingFees = totalFeesExpected - totalCollection;
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-8 font-sans text-slate-900">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
               ...
               <span className="text-[10px] font-black bg-blue-600 text-white px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-blue-100">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select 
              ...
              className="px-4 py-3 rounded-2xl border border-slate-200 bg-white font-black text-[11px] uppercase outline-none shadow-sm cursor-pointer"
            >
            ...
            <div className="flex items-center bg-white border border-slate-200 rounded-2xl shadow-sm p-1.5">
               ...
               <input 
                 type="date" 
                 className="text-[11px] font-black uppercase p-2 outline-none bg-transparent cursor-pointer text-slate-700 w-32"
            ...
            <select
              ...
              className="px-6 py-4 rounded-2xl border-none bg-slate-900 text-white font-black text-[11px] uppercase tracking-widest shadow-xl cursor-pointer outline-none hover:bg-blue-600 transition-all"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-10">
          ...
          <StatCard title="Collection" value={`₹${totalCollection}`} icon={<HiCurrencyRupee/>} color="from-amber-400 to-orange-600" />
          <StatCard title="Pending" value={`₹${pendingFees}`} icon={<HiExclamationCircle/>} color="from-indigo-600 to-purple-800" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 h-fit">
            <h3 className="font-black text-slate-800 uppercase text-xs tracking-[0.2em] mb-8 flex items-center gap-2">
              <span className="w-2 h-6 bg-blue-600 rounded-full"></span> {currentMonth} Snapshot
            </h3>
            <div className="space-y-6">
              <SummaryRow label="Total Students" value={filteredStudents.length} />
              <SummaryRow label="Present Status" value={presentCount} badgeColor="bg-emerald-100 text-emerald-700" />
              <SummaryRow label="Absent Status" value={absentCount} badgeColor="bg-rose-100 text-rose-700" />
              <div className="pt-6 mt-6 border-t border-dashed border-slate-200">
                <SummaryRow label="Total Expected" value={`₹${totalFeesExpected}`} />
                <SummaryRow label="Total Paid" value={`₹${totalCollection}`} valueColor="text-blue-600" />
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
            <h3 className="font-black text-slate-800 uppercase text-xs tracking-[0.2em] mb-8 flex items-center gap-2">
              <span className="w-2 h-6 bg-slate-900 rounded-full"></span> Strength: {selectedSession}
            </h3>
...
const StatCard = ({ title, value, icon, color }) => (
  <div className={`relative overflow-hidden bg-gradient-to-br ${color} p-6 rounded-[2.5rem] text-white shadow-xl transform transition-hover hover:-translate-y-1`}>
