...
const TransferCertificate = () => {
  ...
  const handlePrint = () => {
    // 1. Get the content of the hidden div
    const content = document.getElementById('tc-content').innerHTML;

    // 2. Prepare high-quality styles for the new window
    const printStyles = `
      ...
      <style>
        ...
        .inner-border {
          ...
          height: 100%;
          position: relative;
          background: #fffef2 !important; /* Parchment look */
          box-sizing: border-box;
          -webkit-print-color-adjust: exact; /* Crucial for background color */
        ...
        .handwritten { font-family: 'Pinyon Script', cursive; color: #1e3a8a; font-size: 2rem; }
    ...
    // 4. Create Blob and Open Window
    const blob = new Blob([fullHTML], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    window.open(url, '_blank');
  };

  if (loading) return <div className="p-20 text-center font-bold text-white bg-slate-900 h-screen">Loading Data...</div>;
  if (!student) return <div className="p-20 text-center font-bold text-red-500 bg-slate-900 h-screen">Student Not Found!</div>;

  return (
    <div className="bg-slate-900 min-h-screen py-10 flex flex-col items-center no-print">
      
      {/* Settings Bar */}
      <div className="w-[850px] bg-white p-6 rounded-xl shadow-2xl flex justify-between items-center border-b-4 border-blue-900">
        <div className="flex flex-col">
          <span className="text-[11px] font-black text-blue-900 uppercase">Step 1: Verify PNR Number</span>
          <input 
            value={manualPNR} 
            onChange={(e) => setManualPNR(e.target.value)}
            className="border-b-2 border-blue-100 outline-none text-blue-900 font-bold text-lg w-64 focus:border-blue-900 transition-colors"
            placeholder="Enter PNR/PEN..."
          />
        </div>
        <div className="text-right">
          <p className="text-[10px] text-gray-400 font-bold mb-2 uppercase">Ready to generate official copy?</p>
          <button 
            onClick={handlePrint}
            className="bg-blue-900 text-white px-10 py-3 rounded-lg font-black uppercase tracking-tighter hover:bg-blue-800 transition-all shadow-lg active:scale-95"
          >
            Generate & Print TC
          </button>
        </div>
      ...
      {/* THIS DIV IS HIDDEN ON SCREEN 
          It contains the raw data that handlePrint picks up 
      */}
      ...
      <div className="mt-10 p-10 bg-white/5 rounded-lg border border-white/10 text-center max-w-lg">
        <p className="text-white font-medium">TC Preview is optimized for print.</p>
        <p className="text-slate-400 text-sm mt-2 italic">Official certificate will include ornamental borders and vintage fonts in a new window.</p>
      </div>
    </div>
  );
};
